...
function get_instructions($db, $id){
    try {
    ...
    $statement->execute();
        ...
        echo "Unable to retrieve instructions";
        exit;
    }
    return $statement->fetchALL(PDO::FETCH_ASSOC);
}

function add_instruction($db, $recipe_id, $instruction){
    $query = 'INSERT INTO instructions(recipe_id, instruction)
                VALUES(:recipe_id, :instruction)';
    try{
        $statement = $db->prepare($query);
        $statement->bindParam(':recipe_id', $recipe_id, PDO::PARAM_INT);
        $statement->bindParam(':instruction', $instruction, PDO::PARAM_STR);
        $statement->execute();
    }catch(Exception $e){
        echo "Could not add this instruction. ".$e->getMessage();
        exit;
    }
    return true;
}
...
function add_recipe($db, $title, $subtitle, $img_src, $url ){
    $query = 'INSERT INTO recipe(title, subtitle, img_src, url)
                VALUES(:title, :subtitle, :img_src, :url)';
